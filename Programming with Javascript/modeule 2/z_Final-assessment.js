...
function buildUserSummary(user) {
  ...
  const avg = Array.isArray(user.scores) ? averageScore(user["scores"]) : null;

  return { id: user.id, name: name, scoreCount: scoreCount, avg: avg };
}
...
// 3) "" is a defined, valid value of type string (an intentional empty input),
//    while null/undefined represent the total ABSENCE of a value. Conceptually,
//    null/undefined mean "no data was ever provided," whereas "" means "a value
//    was provided, and that value happens to be empty." In normalizeName we
//    still want "" (and whitespace-only strings) treated as "Unknown" for
//    display purposes, but we detect them separately (via trim() check) rather
//    than lumping them in with null/undefined, since they are logically
//    different cases even though they resolve to the same fallback output here.
